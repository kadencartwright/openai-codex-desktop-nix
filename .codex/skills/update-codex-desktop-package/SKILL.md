...
2. Determine the target OpenAI Codex desktop app version. If the user asks for the latest and no version is provided, verify it from an authoritative source; do not guess because the upstream zip URL has no known public index.
...
This updates `default.nix`, refreshes the official zip hash, and runs `nix build .#openai-codex-desktop`.
